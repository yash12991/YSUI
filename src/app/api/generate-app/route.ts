...
import { callAI } from '@/lib/agents/aiProvider';
import { createProject, createVersion, getLatestVersion } from '@/lib/db/queries';
...
function htmlProjectFiles(title: string, html: string): ProjectFile[] {
  return [
    {
      path: 'README.md',
      content: `# ${title}

Generated by SimplyUI as a standalone web app.
...
`,
    ...
    { path: 'index.html', content: html },
    {
      ...
      content: JSON.stringify({
        name: title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'simplyui-html-app',
        version: '1.0.0',
        private: true,
        scripts: {
          dev: 'npx serve .',
...
export async function POST(request: NextRequest) {
  try {
    const { prompt, userId, projectId: existingProjectId, currentHtml, currentVersion } = await request.json();
    if (!prompt?.trim()) {
      return NextResponse.json({ success: false, error: 'Prompt required' }, { status: 400 });
    }

    const userMessage = `${currentHtml ? `Modify this existing HTML app according to the request. Keep it complete and self-contained.\n\nREQUEST: "${prompt}"\n\nEXISTING HTML:\n${currentHtml.slice(0, 24000)}` : `Build a STUNNING, COMPLETE, FULLY FUNCTIONAL web application for: "${prompt}"`}
...
Output ONLY the complete HTML file — no explanations, no markdown.`;
    ...
    const html = await callAI(userMessage, FULL_APP_SYSTEM_PROMPT);
    ...
    const files = htmlProjectFiles(title || 'SimplyUI app', cleanHtml);
    let projectId: string | undefined;
    let versionNumber = 1;

    if (existingProjectId) {
      const latest = await getLatestVersion(existingProjectId);
      versionNumber = Math.max((latest?.versionNumber || 0) + 1, (currentVersion || 0) + 1);
      await createVersion(existingProjectId, versionNumber, cleanHtml, {
        outputMode: 'html',
        title,
        prompt,
        files,
      ...
      const project = await createProject(
        userId,
        `${title || 'Full app'} ${Date.now()}`,
        prompt,
        { source: 'full-html-generator' }
      );
      await createVersion(project.id, 1, cleanHtml, {
        outputMode: 'html',
        title,
        prompt,
        files,
    ...
    return NextResponse.json({
      success: true,
      data: { html: cleanHtml, title, outputMode: 'html', projectId, versionNumber, files, downloadUrl: projectId ? `/api/projects/${projectId}/download` : undefined },
    });
  } catch (error) {
