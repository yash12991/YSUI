...
import { Sparkles, Eye, EyeOff, Mail, Lock, User, ArrowRight, Github, Globe, Check, X } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const passwordChecks = [
    { label: 'At least 6 characters', check: password.length >= 6, desc: 'Must be at least 6 characters' },
    { label: 'Has uppercase letter', check: /[A-Z]/.test(password), desc: 'Include an uppercase letter' },
    { label: 'Has a number', check: /\d/.test(password), desc: 'Include at least one number' },
    { label: 'Has a symbol', check: /[!@#$%^&*(),.?":{}|<>]/.test(password), desc: 'Include a symbol for strength' },
  ]
  const passwordScore = passwordChecks.filter(c => c.check).length
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{ color: '#8b96a5', fontSize: '0.85rem', lineHeight: 1.7, margin: '0 0 20px' }}>
                The community-powered studio where ideas become production-ready UIs.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {[
                  { icon: '🧩', title: 'Component Studio', desc: 'Generate, customize, and save reusable UI components with AI.' },
                  { icon: '👥', title: 'Community Gallery', desc: 'Publish your creations, browse shared UIs, remix and learn.' },
                  { icon: '💾', title: 'Version Control Built-in', desc: 'Every change tracked. Roll back, fork, or export any version.' },
                  { icon: '🔌', title: 'Multiple AI Providers', desc: 'Groq, Gemini, Minimax M1 — pick the model that works best.' },
                  { icon: '📦', title: 'ZIP Export + GitHub', desc: 'Download production folders or push directly to repos (coming).' },
                ].map((item, i) => (
          ...
          <div style={{
            ...
            <div style={{
              ...
              <p style={{ color: '#8b96a5', fontSize: '0.88rem', lineHeight: 1.6, margin: '0 0 28px' }}>
                Join the community. Generate, share, and remix UIs with AI.
              </p>
              ...
              <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <User size={16} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: '#4a5568', pointerEvents: 'none' }} />
                    <input
                      value={name}
                    ...
                    <div style={{ marginTop: '8px' }}>
                      <div style={{
                        ...
                        <div style={{
                          ...
                          borderRadius: '2px',
                          transition: 'all 0.3s',
                          background: passwordScore <= 1 ? '#ef4444' : passwordScore <= 2 ? '#f59e0b' : passwordScore <= 3 ? '#10b981' : '#22c55e',
                        }} />
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px' }}>
                        {passwordChecks.map((check, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.68rem', color: check.check ? '#22c55e' : '#4a5568', transition: 'color 0.2s' }}>
                            {check.check ? <Check size={10} /> : <X size={10} />}
                            {check.label}
                          </div>
                        ))}
                      </div>
                ...
                <p style={{ color: '#4a5568', fontSize: '0.72rem', textAlign: 'center', margin: '0', lineHeight: 1.5 }}>
                  By creating an account, you agree to our{' '}
                  <a href="#" style={{ color: '#10b981', textDecoration: 'none' }}>Terms of Service</a> and{' '}
